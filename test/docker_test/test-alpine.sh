...
if [ -z "${MODULE_SRC_DIR}" ]; then
    if [ -e "$cwd/src/msgpack-module.cpp" ]; then
        MODULE_SRC_DIR=$cwd
    else
        MODULE_SRC_DIR=$WORKDIR/module-msgpack
    fi
fi
...
cmake .. -DCMAKE_BUILD_TYPE=debug -DCMAKE_INSTALL_PREFIX=${INSTALL_PREFIX} ${CMAKE_EXTRA_FLAGS}
make -j${MAKE_JOBS}
make install
